...
from baseline.evaluations import *
from baseline.initialize_model_DNN import initialize_model_DNN 
###################################################

def parse_args():
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Run DNN baseline")
...
def main():
    ...
    gene_folder_path = os.path.join('Data', CBM, 'Gene_AA')
    metabolite_smiles_path = os.path.join('Data', CBM, 'Target metabolites_smile.csv')
    relationship_folder_path = os.path.join('Data', CBM, 'Label')
    output_csv_path = os.path.join('Data', CBM, 'Results', 'all_metabolites_predictions_temp.csv')
    ###################################################
    ...
    model, _ = initialize_model_DNN(gene_vocab_size, smiles_vocab_size, max_len, smiles_max_len, use_cpu=1)
    model.to(device)
    # Load the model weights from the current directory
    model.load_state_dict(torch.load(f'baseline/DNN_{CBM}_Gene_AA.sav',map_location=device))
    print("Model loaded!")
